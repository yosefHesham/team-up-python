...
const base_url = 'https://c4c2d2de-af5f-4a37-b685-424eae62c49b-ide.cs50.xyz:8080'
...
$('#name-form').submit(function(e) {
  ...
  if (name.length) {
    ...
    fetch(`${base_url}/registration?name=${name}`)
...
$('#skills-form').submit(function(e) {
  ...
  if (skills.length >= 1) {
    ...
    fetch(`${base_url}/registration?skills=${user_skills.join(',')}`)
      .then(response => response.json())
      .then(students => show_students(students, section_id))
      .catch(err => show_nothing(section_id));
  } else {
...
function show_students(students_list, section_id) {
  ...
  for (let student of students_list) {
    ...
    for (let skill of student.skills) {
      skills += `<li class="topic-tag-action f6 p-2 float-left js-tag-input-tag text-blue" style="height: 25px">${skill.split(",")}</li>`;
    }
    list_in_html += `<div class="ui fluid container mb-2 p-2 text-white rounded student" >
        ...
        <ul class=" js-tag-input-selected-tags">${skills.split(",")}</ul></div>`;
  }
